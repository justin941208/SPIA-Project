...
def run_all_props_with_current_settings(passes, timeout):
    ...
    for prop in vprops:
        ...
        # run the searcher
        searcher.run(passes, multi=False, threads=threads, print_output=False, clear_output=False)
...
timeout = int(eval(input("timeout (min)? ")))
passes = int(eval(input("num passes? ")))
beams = int(eval(input("search beam width? ")))
hyp_bonus = 1.0*int(eval(input("hyp bonus? (0 in the paper, but 10 may give better results)")))
